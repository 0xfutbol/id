...
const RPC_URL = networkConfig.matchain.rpcEndpoint;
const MAX_RETRIES = 3;
...
export class MatchainService implements BlockchainService {
  ...
  async getNFTsByOwner(params: {
    ...
    try {
      ...
      // Get balance
      const balance = await this.retryContractCall(() => contract.balanceOf(walletAddress));
      ...
      for (let i = 0; i < balanceNumber; i++) {
        try {
          ...
          console.error(`Error fetching token ${i} for ${walletAddress} on ${this.chainName}:`, error instanceof Error ? error.message : String(error));
        }
      }
