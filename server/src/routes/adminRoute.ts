...
import { getDiscordAccountByAddress, getReferralCount, getTonAccountByAddress } from '../repo/db';
...
adminRouter.get('/account/:address', authenticateAdmin, async (req: express.Request & { user?: { owner: string } }, res) => {
  try {
    ...
    const tonAccount = await getTonAccountByAddress(req.params.address);
